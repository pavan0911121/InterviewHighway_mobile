...
const LessonDetails = () => {
  ...
  const handleDownload = async (documentId: string) => {
    ...
    try {
      ...
      const res = await config({ fileCache: true, path: filePath }).fetch('GET', downloadUrl, authHeader as any)
      const localPath = res.path()
      setDownloadedPdfPaths(prev => ({ ...prev, [documentId]: localPath }))
    } catch (error) {
  ...
  const handleOpenDocument = async (documentId: string) => {
    ...
    try {
      ...
      const exists = await BlobUtil.fs.exists(normalizedPath)
      if (!exists) {
        console.error('Open document failed: file does not exist', normalizedPath)
        return
      }
